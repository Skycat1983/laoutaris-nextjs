...
import helpers from "./create-shopify-catalog-pilot-helpers.cjs";
...
const createPilotProduct = async ({ endpoint, token, variables }) => {
...
try {
  ...
  const outputPath = args.output ?? DEFAULT_PILOT_OUTPUT_PATH;
  const [
  ...
  for (let index = 0; index < report.products.length; index += 1) {
    ...
    try {
      const payload = await createPilotProduct({
        endpoint,
  ...
  console.log("Shopify catalog pilot creation report written.");
  ...
  exitCode = hasPilotCreateFailures(report) ? 1 : 0;
} catch (error) {
  exitCode = 1;
  console.error("Shopify catalog pilot creation failed.");
