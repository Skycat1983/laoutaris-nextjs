...
| F-030 | A-007, A-013 | Medium | Partially mitigated | Route URL ownership is split across helpers and hard-coded paths. | T-087 deleted the retired server-side same-app API wrappers that owned the remaining `VERCEL_ENV`/`VERCEL_URL`/localhost server self-fetch URL construction. T-086 removed hard-coded same-app origins from `LogoutForm`, `MobileNavDrawer`, and the `/project` redirect by using relative app paths. T-085 removed the shop listing loader's `NEXT_PUBLIC_BASE_URL`/localhost dependency. T-302 completed a source-backed route-builder inventory. T-305 added the first client-safe public app route builder module for scoped public roots/detail paths plus metadata/sitemap/main-nav/footer consumers, and T-307 expanded those builders across low-risk public cards, blog sections, masonry/detail links, and shop linked-artwork links. T-308 scoped the first auth/protected route constants implementation, and T-311 completed that runtime slice for middleware, route utils, NextAuth sign-in config, and admin frontend redirects. T-309 scoped API route-builder ownership, T-312 completed admin delete client fetcher URL builders, and T-314 completed admin read client fetcher URL/query builders while keeping physical routes and request-context route IDs separate. T-315 scoped account/admin route-builder ownership, and T-316 completed the account UI route-builder slice. T-317 scoped the next API family to admin update client fetcher URLs; T-318 completed that slice; T-319 completed admin dashboard UI route builders; and T-320 completed admin create API path builders. T-321 scoped release-fixture, crawler, callback, and saved-item route ownership. T-322 completed saved-item revalidation path-builder usage, and T-323/T-324 scoped then covered the current auth redirect callback contract without changing `/dashboard`. Remaining route ownership work is broader public/user API route families, root-layout session/cache ownership, any future decided auth callback destination change, and broader architecture refactors. Smoke/sitemap/robots release fixtures remain explicit for independent evidence. | [architecture workstream](../workstreams/architecture-refactor-and-code-health.md), [frontend workstream](../workstreams/frontend-routes-and-components.md), [Shopify workstream](../workstreams/shopify-commerce.md), [deployment workstream](../workstreams/deployment-security-and-observability.md), [T-086](../tasks/T-086-remove-hard-coded-localhost-navigation-urls.md), [T-087](../tasks/T-087-retire-server-api-self-fetch-wrappers.md), [T-302 result](results/T-302-route-builder-centralization-scope.md), [T-305](../tasks/T-305-add-public-app-route-builders.md), [T-307](../tasks/T-307-expand-public-route-builder-consumers.md), [T-308 result](results/T-308-auth-protected-route-constants-scope.md), [T-309 result](results/T-309-api-route-builder-ownership-scope.md), [T-311](../tasks/T-311-centralize-auth-protected-route-constants.md), [T-312](../tasks/T-312-add-admin-delete-api-path-builders.md), [T-314](../tasks/T-314-add-admin-read-api-path-builders.md), [T-315 result](results/T-315-account-admin-route-builder-ownership-scope.md), [T-316](../tasks/T-316-add-account-ui-route-builders.md), [T-317 result](results/T-317-next-api-route-builder-family-scope.md), [T-318](../tasks/T-318-add-admin-update-api-path-builders.md), [T-319](../tasks/T-319-add-admin-dashboard-ui-route-builders.md), [T-320](../tasks/T-320-add-admin-create-api-path-builders.md), [T-321](../tasks/T-321-scope-release-fixture-auth-callback-route-ownership.md), [T-322](../tasks/T-322-add-saved-item-revalidation-route-builders.md), [T-323](../tasks/T-323-scope-auth-callback-redirect-contract.md), [T-324](../tasks/T-324-add-auth-callback-redirect-coverage.md) | T-322/T-324 reconciled 2026-05-28 |
| F-031 | A-014 | Medium | Resolved | High-confidence unused leaf files, WIP variants, barrels, starter assets, and import cleanup need staged pruning with verification. | Resolved by T-299 and T-300 on 2026-05-26: T-299 deleted the scoped unused source leaves, WIP/variant files, unused barrels, starter SVG assets, and stale test/comment references after fresh reference checks. T-300 then removed the unreferenced `filterWatchlerlist` helper plus collection layout comment/import leftovers, and re-confirmed saved-item action `revalidatePath` and button state type imports are live. Auth/session, i18n, package, root Shopify note, route behavior, commerce, CI, and owner-decision cleanup remain separate. | [production risks](../risks/production-readiness.md), [architecture workstream](../workstreams/architecture-refactor-and-code-health.md), [T-299](../tasks/T-299-prune-high-confidence-unused-source-leaves.md), [T-300](../tasks/T-300-prune-remaining-a014-utility-import-cleanup.md) | T-300 completed 2026-05-26 |
...
| F-052 | A-008 | Medium | Partially mitigated | Global API CORS and CSP policy are too broad for the production admin, auth, and commerce surface. | T-096 removed the global API CORS header rule from `next.config.mjs`, eliminating the wildcard-origin plus credential pairing and wildcard allowed request headers, and added baseline hardening headers plus low-risk CSP `object-src`, `base-uri`, `form-action`, and `frame-ancestors` directives. T-301 completed a source-backed CSP allowlist inventory, and T-304 added the tightened allowlist as `Content-Security-Policy-Report-Only` with focused tests proving the enforced CSP remains unchanged. Strict enforcement, dynamic route-level CORS, HSTS, CSP reporting, and monitoring follow-ups remain separate. | [A-008 result](results/A-008-security-headers-cors-logging.md), [T-096](../tasks/T-096-harden-baseline-security-headers-cors.md), [T-301 result](results/T-301-csp-allowlist-tightening-scope.md), [T-304](../tasks/T-304-add-report-only-csp-allowlist.md), [production risks](../risks/production-readiness.md), [deployment workstream](../workstreams/deployment-security-and-observability.md) | T-304 completed 2026-05-26 |
...
| F-080 | A-021 | High | Partially mitigated | Production monitoring and error reporting are not implemented. | A-021 found no monitoring/error-reporting dependency, no `instrumentation.ts`, no monitoring env contract, and client/server error surfaces only log to console. T-123 added the decision-ready monitoring/error-reporting architecture plan with required capture surfaces, T-099 request ID and structured-log integration points, provider decision questions, environment-variable classification rules, and the post-approval implementation contract. T-124 added unauthenticated public smoke workflow automation separately. T-139 added ADR 0005 to record the blocked owner/platform decision state. On 2026-05-27 the owner approved Sentry, ADR 0005 was updated to accepted, and T-310 added the first Sentry SDK/instrumentation baseline with environment classification, PII disabled, source-map upload disabled, broad tracing/replay/profiling omitted, App Router error capture, and redacted structured logger forwarding. T-313 restored the broad typecheck/Jest gate after the Sentry/route-builder wave. Remaining monitoring work includes alert automation, owner matrix completion, credentialed/admin smoke, source-map/release policy, replay/profiling/broad tracing decisions, uptime checks, and provider-backed monitoring evidence. | [A-021 result](results/A-021-observability-incident-response.md), [monitoring architecture](../architecture/monitoring-and-error-reporting.md), [ADR 0005](../decisions/0005-monitoring-provider-decision.md), [production risks](../risks/production-readiness.md), [deployment workstream](../workstreams/deployment-security-and-observability.md), [T-123](../tasks/T-123-define-monitoring-provider-plan.md), [T-124](../tasks/T-124-add-public-smoke-github-actions-workflow.md), [T-139](../tasks/T-139-record-monitoring-provider-decision.md), [T-310](../tasks/T-310-implement-sentry-monitoring-baseline.md), [T-313](../tasks/T-313-repair-post-route-builder-verification-drift.md) | T-310/T-313 completed 2026-05-27 |
...
| F-101 | A-017 | Medium | Resolved | Main navigation fails closed when dynamic biography or collection nav data is unavailable. | Resolved by T-147 and simplified by T-283: `MainNavLoader` now renders stable `/biography` and `/collections` route-root links without depending on dynamic nav data in the global header. | [A-017 result](results/A-017-search-navigation-discovery.md), [frontend workstream](../workstreams/frontend-routes-and-components.md), [testing workstream](../workstreams/testing-and-quality.md), [T-147](../tasks/T-147-render-resilient-main-nav-fallbacks.md), [T-283](../tasks/T-283-make-root-header-nav-build-static-safe.md) | T-147 completed 2026-05-19; T-283 updated main nav ownership 2026-05-25 |
...
| F-104 | A-018, A-020 | Low | Resolved | Footer placeholder social links, stale copyright text, and commerce assurance copy need owner-approved cleanup. | Resolved by T-209, T-213, and T-218. T-209 removed unsupported shared-banner commerce assurance copy. T-213 added footer legal links to `/privacy` and `/terms`. T-218 removed the footer's Facebook/Twitter/Instagram `href="#"` placeholders without inventing real social URLs, preserved contact and legal links, and replaced the fixed 2024 copyright with current-year text. Real social URL wiring remains blocked until owner-managed targets are supplied. | F-078, [A-018 result](results/A-018-translations-content-taxonomy.md), [A-020 result](results/A-020-privacy-consent-commerce-compliance.md), [frontend workstream](../workstreams/frontend-routes-and-components.md), [content/admin workstream](../workstreams/content-assets-and-admin-ops.md), [T-209](../tasks/T-209-align-commerce-assurance-copy.md), [T-213](../tasks/T-213-add-owner-approved-policy-routes-and-footer-links.md), [T-218](../tasks/T-218-remove-footer-placeholder-social-links.md) | T-218 completed 2026-05-22 |
...
| F-110 | A-005 | Low | Resolved | Route loading states need a documented public-route fallback pattern. | Resolved by T-207: `docs/architecture/rendering-and-data-fetching.md` now documents the accepted public route loading/fallback pattern for route-level `loading.tsx`, visible public-detail Suspense fallbacks, layout navigation/pagination fallbacks, intentional invisible JSON-LD `fallback={null}` boundaries, and client follow-up loading states. `/project/aims` now uses a route-local neutral desktop image fallback instead of generic blue inline `Loading...` copy, with focused source/documentation coverage. | [A-005 result](results/A-005-frontend-routes-components.md), [frontend workstream](../workstreams/frontend-routes-and-components.md), [testing workstream](../workstreams/testing-and-quality.md), [T-207](../tasks/T-207-document-route-fallback-patterns.md) | T-207 completed 2026-05-23 |
| F-111 | A-022 | High | Partially mitigated | Public archive, blog, search, shop, detail, and dynamic metadata routes are intentionally dynamic and do not yet use ISR, cached non-`fetch` data functions, cache tags, or `generateStaticParams()`. | T-232 through T-251 completed the accepted freshness matrix and the low-risk public cache proofs for biography, collections, sitemap, blog primary/detail/list slices, sorted blog pages, and fixed unfiltered `/artwork` browse pages 1-5. T-252 paused F-111 runtime cache expansion because the remaining candidates carry higher personalization, query-cardinality, Shopify, or mutation-revalidation risk than the completed fixed public list proofs. T-253 then selected T-254 as the next F-115 runtime slice, T-254 completed the modal-provider cleanup/lazy-host proof, T-255 selected T-256 for a provider-preserving public account-navigation lazy island, and T-256 completed that account-navigation split. Comments, `popular` pages beyond page 1, sorted page 6-plus, blog routes, public blog APIs, filtered artwork variants, artwork detail, public artwork APIs, search, shop routes, route-level ISR beyond accepted defaults, and cache tags remain dynamic/deferred. | [A-022 result](results/A-022-nextjs-feature-utilization.md), [production risks](../risks/production-readiness.md), [rendering architecture](../architecture/rendering-and-data-fetching.md), [architecture workstream](../workstreams/architecture-refactor-and-code-health.md), [frontend workstream](../workstreams/frontend-routes-and-components.md), [T-232](../tasks/T-232-define-public-cache-freshness-policy.md), [T-233](../tasks/T-233-pilot-biography-cache-proof.md), [T-237](../tasks/T-237-scope-next-nextjs-efficiency-wave.md), [T-238](../tasks/T-238-pilot-collections-redirect-cache-proof.md), [T-239](../tasks/T-239-codify-sitemap-isr-freshness.md), [T-240](../tasks/T-240-scope-blog-primary-data-cache-split.md), [T-241](../tasks/T-241-pilot-blog-primary-detail-cache-proof.md), [T-242](../tasks/T-242-pilot-default-blog-list-cache-proof.md), [T-243](../tasks/T-243-scope-sorted-blog-list-cache-and-query-hygiene.md), [T-244](../tasks/T-244-normalize-public-blog-list-query-parsing-and-bounds.md), [T-245](../tasks/T-245-pilot-sorted-blog-first-page-cache-proof.md), [T-246](../tasks/T-246-scope-bounded-sorted-blog-page-cache-expansion.md), [T-247](../tasks/T-247-pilot-bounded-sorted-blog-page-cache-proof.md), [T-248](../tasks/T-248-scope-next-public-cache-target.md), [T-249](../tasks/T-249-pilot-default-artwork-browse-cache-proof.md), [T-250](../tasks/T-250-scope-next-cache-efficiency-wave.md), [T-251](../tasks/T-251-pilot-bounded-artwork-browse-cache-expansion.md), [T-252](../tasks/T-252-scope-post-browse-cache-or-client-island-wave.md), [T-253](../tasks/T-253-scope-root-provider-modal-client-island-split.md), [T-254](../tasks/T-254-split-modal-context-language-state-and-lazy-host.md), [T-255](../tasks/T-255-scope-post-modal-provider-efficiency-wave.md), [T-256](../tasks/T-256-lazy-load-public-account-navigation-island.md) | T-256 completed 2026-05-24 |
| F-112 | A-022 | High | Resolved | The current client import graph had regressed and could pull server-only/model/mixed-barrel modules into a client component. | Resolved by T-230: `SignUpForm` now imports the account privacy acknowledgement constants directly from `@/lib/constants/accountPrivacyAcknowledgement` instead of the broad `@/lib/constants` barrel. Focused import-boundary and sign-up privacy tests plus `npm run build` passed. | [A-022 result](results/A-022-nextjs-feature-utilization.md), [production risks](../risks/production-readiness.md), [architecture workstream](../workstreams/architecture-refactor-and-code-health.md), [frontend workstream](../workstreams/frontend-routes-and-components.md), [T-230](../tasks/T-230-fix-client-import-boundary-regression.md) | T-230 completed 2026-05-23 |
| F-113 | A-022 | High | Resolved | Middleware still matched most public paths before returning early, leaving avoidable request-time work and future proxy migration risk. | Resolved by T-231 on 2026-05-23: `src/middleware.ts` now matches only `/account`, `/admin`, `/api/v2/admin`, and `/api/v2/user`; focused tests prove public pages, public APIs, `/api/auth/*`, and protected-prefix lookalikes are outside the matcher contract. | [A-022 result](results/A-022-nextjs-feature-utilization.md), [production risks](../risks/production-readiness.md), [architecture workstream](../workstreams/architecture-refactor-and-code-health.md), [deployment workstream](../workstreams/deployment-security-and-observability.md), [T-231](../tasks/T-231-narrow-protected-middleware-matcher.md) | T-231 completed 2026-05-23 |
| F-114 | A-022 | High | Resolved | `sitemap.ts`, `/biography`, and `/collections` are statically generated while reading mutable MongoDB/Shopify data, so their freshness was implicit and deploy-bound. | Resolved by T-232: the rendering architecture now explicitly records current deploy-bound freshness for sitemap/default redirects and the accepted later targets: approximately 1-hour ISR for sitemap and approximately 10-minute ISR for default redirects after the biography proof. Runtime source behavior was intentionally unchanged. | [A-022 result](results/A-022-nextjs-feature-utilization.md), [production risks](../risks/production-readiness.md), [rendering architecture](../architecture/rendering-and-data-fetching.md), [deployment workstream](../workstreams/deployment-security-and-observability.md), [T-232](../tasks/T-232-define-public-cache-freshness-policy.md) | T-232 completed 2026-05-23 |
| F-115 | A-022 | Medium | Partially mitigated | Global root client providers and large public client islands reduce how much public UI can stay purely server-rendered. | T-234 measured the current provider/client-island cost and found the root layout client chunk still carried cross-cutting session, modal, search drawer, and mobile navigation drawer code. T-236 then lazy-loaded the public mobile search/navigation drawer bodies while preserving root provider placement; build evidence showed the root layout client chunk drop from 35,174 to 27,772 bytes, with separate lazy drawer chunks emitted. T-254 removed unused language state from the active root modal provider path and lazy-loaded the modal dialog host while preserving global session and modal provider ownership. T-256 deferred public header account menu/dropdown implementation code until account-menu intent while preserving rooted session/modal providers; the root layout chunk measured 22,313 bytes uncompressed versus the T-254 27,498 byte baseline. | [A-022 result](results/A-022-nextjs-feature-utilization.md), [architecture workstream](../workstreams/architecture-refactor-and-code-health.md), [frontend workstream](../workstreams/frontend-routes-and-components.md), [T-234](../tasks/T-234-scope-public-client-provider-islands.md), [T-236](../tasks/T-236-lazy-load-public-mobile-navigation-drawers.md), [T-252](../tasks/T-252-scope-post-browse-cache-or-client-island-wave.md), [T-253](../tasks/T-253-scope-root-provider-modal-client-island-split.md), [T-254](../tasks/T-254-split-modal-context-language-state-and-lazy-host.md), [T-255](../tasks/T-255-scope-post-modal-provider-efficiency-wave.md), [T-256](../tasks/T-256-lazy-load-public-account-navigation-island.md) | T-256 completed 2026-05-24 |
| F-116 | A-022 | Medium | Duplicate | Next.js instrumentation and web-vitals reporting remain absent, but this is already owned by the monitoring provider decision track. | A-022 found no `instrumentation.ts`, `useReportWebVitals`, monitoring SDK, OpenTelemetry, Sentry, or web-vitals package usage. F-080/R-019/ADR 0005 own the monitoring track; Sentry is approved and T-310 added the first Next instrumentation/Sentry baseline. Web-vitals reporting, provider smoke, release evidence, source-map upload, alerts, replay, profiling, broad tracing, and uptime checks remain outside this duplicate finding. | F-080, [A-022 result](results/A-022-nextjs-feature-utilization.md), [production risks](../risks/production-readiness.md), [ADR 0005](../decisions/0005-monitoring-provider-decision.md), [deployment workstream](../workstreams/deployment-security-and-observability.md), [T-310](../tasks/T-310-implement-sentry-monitoring-baseline.md) | T-310 completed 2026-05-27 |
| F-117 | A-022 | Low | Resolved | Top-level `"use server"` appears in server component, loader, and layout files where `server-only` or ordinary Server Components would better express intent. | Resolved by T-235: ordinary App Router layouts/pages, server component loaders, navigation components, public views, prototype loader helpers, and ordinary utility modules no longer use top-level `"use server"` as a Server Component marker. Remaining directives are limited to `src/lib/actions` and `src/lib/session`, and `clientServerImportBoundary` coverage guards against non-action reintroduction. | [A-022 result](results/A-022-nextjs-feature-utilization.md), [architecture workstream](../workstreams/architecture-refactor-and-code-health.md), [frontend workstream](../workstreams/frontend-routes-and-components.md), [T-235](../tasks/T-235-remove-non-action-use-server-directives.md) | T-235 completed 2026-05-23 |
| F-118 | A-024 | High | Resolved | Dynamic public route transitions need segment-specific loading shells for artwork, search, and shop product browsing. | A-024 found only the root `src/app/loading.tsx` generic spinner, no route-local loading file or route-shaped Suspense fallback for `/artwork`, `/search`, or `/shop/products`, and better existing coverage on `/blog` through a section skeleton. A-024 phases 2 and 3 added route-local loading shells for `/artwork`, `/search`, and `/shop/products`. | [A-024 result](results/A-024-loading-state-ux.md), [frontend workstream](../workstreams/frontend-routes-and-components.md), [architecture workstream](../workstreams/architecture-refactor-and-code-health.md), [testing workstream](../workstreams/testing-and-quality.md) | A-024 phases 2-3 completed 2026-05-24 |
| F-119 | A-024 | Medium | Resolved | Programmatic search, artwork, and account navigations need pending feedback and should avoid full document navigation for internal routes. | A-024 found `Searchbar` and `SearchDrawerBody` call `router.push()` without pending feedback, the home artwork filter hero uses `window.location.href` for internal artwork search URLs, and account dropdown internal routes use raw anchors. T-256 moved account dropdown implementation behind an account-menu lazy island. T-257 completed the scoped `Searchbar`, `SearchDrawerBody`, and home artwork filter hero pending-feedback slice. T-260 replaced the remaining account-dropdown internal anchors with App Router `Link` semantics while preserving disabled menu states, logout modal behavior, and the lazy account island. | [A-024 result](results/A-024-loading-state-ux.md), [frontend workstream](../workstreams/frontend-routes-and-components.md), [T-257](../tasks/T-257-add-programmatic-navigation-pending-feedback.md), [T-260](../tasks/T-260-align-account-menu-internal-navigation.md) | T-260 completed 2026-05-24 |
| F-120 | A-024 | Medium | Resolved | Shared spinner and loading primitives need accessible status semantics and better public-route layout preservation. | A-024 found `PageLoading` renders full-viewport generic `Loading...` copy, `Spinner` lacks `role="status"`/`aria-live`/hidden status text, and shop/artwork localized spinners do not consistently mark busy regions. A-024 Phase 1 added `LoadingStatus`, upgraded shared spinner/page/submit loading semantics, and converted low-risk public in-page loaders. | [A-024 result](results/A-024-loading-state-ux.md), [frontend workstream](../workstreams/frontend-routes-and-components.md), [testing workstream](../workstreams/testing-and-quality.md) | A-024 Phase 1 completed 2026-05-24 |
| F-121 | A-024 | Medium | Resolved | Admin dashboard `@main` parallel route needs a matching loading skeleton. | A-024 found `src/app/admin/dashboard/@feed/loading.tsx` renders `FeedSkeleton`, while there was no `src/app/admin/dashboard/@main/loading.tsx`; T-258 added the matching `@main` route loading skeleton and removed stale generic loading imports from the default route. | [A-024 result](results/A-024-loading-state-ux.md), [frontend workstream](../workstreams/frontend-routes-and-components.md), [auth/admin workstream](../workstreams/auth-admin-and-permissions.md), [testing workstream](../workstreams/testing-and-quality.md), [T-258](../tasks/T-258-add-admin-main-loading-skeleton.md) | T-258 completed 2026-05-24 |
| F-122 | A-024 | Low | Resolved | Route-transition loading behavior needs a targeted browser or component navigation check after loading shells exist. | A-024 found existing source/unit tests for local fallback patterns and client follow-up loading states, but no delayed route-transition check for `/artwork`, `/search`, or `/shop/products`. T-259 added a focused component-level delayed route-transition check that submits the real desktop search control, renders the route-local `/search` loading shell during suspension, and verifies final content replaces it. | [A-024 result](results/A-024-loading-state-ux.md), [testing workstream](../workstreams/testing-and-quality.md), [frontend workstream](../workstreams/frontend-routes-and-components.md), [T-259](../tasks/T-259-add-targeted-route-transition-loading-check.md) | T-259 completed 2026-05-24 |
| F-123 | A-027, A-034 | High | Resolved | Full Jest is not currently a usable release gate under the pinned runtime. | Resolved locally by T-276, T-277, T-280, T-282, and T-286: deterministic source-contract drift, admin full-run timeout policy, and the sandbox socket artifact were repaired; T-282 then passed full `npm test` under Node `22.14.0`; T-286 kept full Jest in the repo-owned `npm run verify:local` aggregate. Main CI ownership remains separate under F-126. | [A-027 result](results/A-027-verification-gate-snapshot.md), [A-034 result](results/A-034-current-jest-failure-triage.md), [testing workstream](../workstreams/testing-and-quality.md), [T-276](../tasks/T-276-fix-deterministic-jest-assertion-drift.md), [T-277](../tasks/T-277-stabilize-admin-jest-full-run-timeouts.md), [T-280](../tasks/T-280-handle-public-smoke-socket-sandbox-path.md), [T-282](../tasks/T-282-run-post-repair-verification-snapshot.md), [T-286](../tasks/T-286-own-local-verification-gate-scripts.md), [production risks](../risks/production-readiness.md) | T-286 completed 2026-05-25 |
| F-124 | A-027, A-035 | High | Partially mitigated | `npm run build` still needs live external data access during static generation, with accidental hard-failure paths and intentional ISR/static evidence requirements. | A-027 found sandbox build failed on MongoDB SRV DNS and passed with external network access. A-035 mapped accidental hard failures on `/account*` and `/project/about`, plus intentional external data surfaces for `/biography`, `/collections`, and `/sitemap.xml`; `/prototype/home` also performed live reads during production build. T-278 removed the accidental `/account*` and `/project/about` hard failures, T-279 documented the intentional external-build evidence policy, T-283 removed the root-header navigation live-read path, and T-284 made `/prototype/home` explicitly dynamic so prototype section loaders no longer run during production static generation. Intentional external evidence remains for `/biography`, `/collections`, and `/sitemap.xml`. | [A-027 result](results/A-027-verification-gate-snapshot.md), [A-035 result](results/A-035-build-external-dependency-map.md), [deployment workstream](../workstreams/deployment-security-and-observability.md), [architecture workstream](../workstreams/architecture-refactor-and-code-health.md), [T-278](../tasks/T-278-isolate-account-project-build-hard-failures.md), [T-279](../tasks/T-279-document-external-build-evidence-policy.md), [T-283](../tasks/T-283-make-root-header-nav-build-static-safe.md), [T-284](../tasks/T-284-set-prototype-home-production-build-policy.md), [production risks](../risks/production-readiness.md) | T-284 completed 2026-05-25 |
| F-125 | A-027 | Medium | Resolved | Explicit TypeScript checking is useful but not yet an owned gate. | Resolved locally by T-268, T-275, T-285, and T-286: explicit `tsc --noEmit` diagnostics were repaired, the final T-285 lazy MongoDB client test typing regression passed, and T-286 added `npm run typecheck` plus the `npm run verify:local` aggregate. CI ownership remains separate under F-126. | [A-027 result](results/A-027-verification-gate-snapshot.md), [testing workstream](../workstreams/testing-and-quality.md), [T-268](../tasks/T-268-fix-explicit-typecheck-test-errors.md), [T-275](../tasks/T-275-restore-explicit-typecheck-after-admin-guard.md), [T-285](../tasks/T-285-fix-client-promise-node-env-typecheck.md), [T-286](../tasks/T-286-own-local-verification-gate-scripts.md) | T-286 completed 2026-05-25 |
| F-126 | A-027, A-033 | Medium | Partially mitigated | Repository-owned CI and smoke evidence did not cover the full local verification or release-smoke baseline. | A-027 found only the public-smoke workflow existed and it did not run install/env/Jest/lint/build/typecheck. T-286 added repo-owned local verification scripts and T-288 added the non-secret Main CI workflow for typecheck, full Jest, build, lint, and event-aware whitespace checks. A-033 confirmed public smoke is unauthenticated and scheduled/detail coverage depends on owner-provided repo variables; credential/admin smoke and Vercel log evidence remain manual and owner-blocked. On 2026-05-28 the owner configured `SMOKE_BASE_URL` and approved optional public smoke values; T-325 aligned the product not-found check with live route-local not-found UI and verified the approved production values with 16 passed, 0 failed, and 0 skipped. | [A-027 result](results/A-027-verification-gate-snapshot.md), [A-033 result](results/A-033-deployment-monitoring-smoke.md), [testing workstream](../workstreams/testing-and-quality.md), [deployment workstream](../workstreams/deployment-security-and-observability.md), [T-274](../tasks/T-274-prepare-production-ops-owner-decision-packet.md), [T-286](../tasks/T-286-own-local-verification-gate-scripts.md), [T-288](../tasks/T-288-add-main-ci-local-gate-workflow.md), [T-325](../tasks/T-325-align-public-smoke-product-not-found-check.md), [production risks](../risks/production-readiness.md) | T-325 completed 2026-05-28 |
| F-127 | A-027 | Medium | Converted | Default local Node/npm can produce non-baseline verification evidence unless agents activate the pinned runtime. | A-027 observed the default shell at Node `21.2.0` and npm `10.2.3`, while repo docs and package metadata require Node `22.14.0` and npm `10.9.2`. | [A-027 result](results/A-027-verification-gate-snapshot.md), [testing workstream](../workstreams/testing-and-quality.md), [testing runbook](../runbooks/testing.md) | Reconciled by T-267 on 2026-05-25 |
| F-128 | A-028 | High | Resolved | Deep-linked `/artwork` browse pages lost server pagination metadata before client load-more state. | A-028 found `ArtworkListLoader` passed only artwork data and `ArtworkGallery` always started at page 1 with `hasMore=true`. T-265 now passes list metadata to the gallery, initializes `page`/`hasMore` from server metadata, loads page 4 after a direct page-3 render, and stops loading on terminal pages. | [A-028 result](results/A-028-public-shopify-hotspots.md), [frontend workstream](../workstreams/frontend-routes-and-components.md), [testing workstream](../workstreams/testing-and-quality.md), [T-265](../tasks/T-265-fix-artwork-browse-pagination-state.md) | T-265 completed 2026-05-25 |
| F-129 | A-028 | High | Resolved | Untyped public search was coupled to full Shopify product-list fan-out. | A-028 found all-type `/search` called `getShopProductList()` and fanned out through Shopify product reads before in-memory filtering. T-266 changed untyped search to MongoDB-backed archive content only and kept shop products behind explicit `type=shop-products`, with unavailable-product metadata when explicit Shopify lookup fails. | [A-028 result](results/A-028-public-shopify-hotspots.md), [frontend workstream](../workstreams/frontend-routes-and-components.md), [Shopify workstream](../workstreams/shopify-commerce.md), [testing workstream](../workstreams/testing-and-quality.md), [T-266](../tasks/T-266-bound-public-search-shopify-product-search.md) | T-266 completed 2026-05-25 |
| F-130 | A-028 | Medium | Resolved | Public shop sort/filter controls drifted from route/API contracts and canonical taxonomy values. | A-028 found `sortBy` accepted by schema but not wired through page/service/API/URL updates, and shop filters omitted canonical values such as `2020s`, `paint`, and `pastel`. T-269 made `sortBy` route-backed across page, loader, service, API, and client URL updates, and derived filter options from canonical artwork constants. | [A-028 result](results/A-028-public-shopify-hotspots.md), [Shopify workstream](../workstreams/shopify-commerce.md), [frontend workstream](../workstreams/frontend-routes-and-components.md), [T-269](../tasks/T-269-align-shop-listing-sort-and-filter-contracts.md) | T-269 completed 2026-05-25 |
| F-131 | A-028 | Medium | Converted | Product-detail frame/mat/gallery controls are preview-only and need owner review before they affect purchase, enquiry, variant, price, or persistence behavior. | A-028 confirmed T-261 sale-gallery controls keep state local, architecture documents the preview-only boundary, and enquiry carries only product-handle context. T-293 through T-297 completed the scoped sale-gallery follow-up chain: the named print and original routes are owner-review-ready for the current visual direction, the book candidate remains blocked until Shopify Storefront data exposes durable book metadata, and the owner packet now gates any rail adoption, material assets, Shopify option mapping, physical dimensions, cart/checkout, enquiry mutation, or repeat book QA. | [A-028 result](results/A-028-public-shopify-hotspots.md), [Shopify workstream](../workstreams/shopify-commerce.md), [Shopify commerce architecture](../architecture/shopify-commerce.md), [sale-gallery owner review packet](../prototypes/sale-gallery-owner-review-packet.md), [T-261](../tasks/T-261-build-shop-product-sale-gallery-mockup.md), [T-293](../tasks/T-293-review-live-sale-gallery-visual-qa.md), [T-294](../tasks/T-294-harden-sale-gallery-product-kind-classification.md), [T-295](../tasks/T-295-recheck-original-sale-gallery-visual-qa.md), [T-296](../tasks/T-296-define-shopify-book-metadata-readiness.md), [T-297](../tasks/T-297-prepare-sale-gallery-owner-review-packet.md), [T-298](../tasks/T-298-reconcile-sale-gallery-owner-review-trackers.md) | T-298 reconciled 2026-05-26 |
| F-132 | A-028 | Medium | Resolved | Product detail injected plain Shopify descriptions as HTML without a rich-description sanitizer decision. | A-028 found `ShopProductSaleGallery` used `dangerouslySetInnerHTML` on `product.description`. T-270 now renders plain descriptions as React text while keeping sanitized rich `descriptionHtml` rendering out of scope. | [A-028 result](results/A-028-public-shopify-hotspots.md), [frontend workstream](../workstreams/frontend-routes-and-components.md), [Shopify workstream](../workstreams/shopify-commerce.md), [T-270](../tasks/T-270-fix-public-route-correctness-polish.md) | T-270 completed 2026-05-25 |
| F-133 | A-028 | Medium | Resolved | Missing `/collections/[slug]` redirect targets used the public error path instead of not-found behavior. | A-028 found missing collection navigation data was rethrown as a generic route error. T-270 now calls route-local `notFound()` for missing redirect targets while preserving service failures as route errors. | [A-028 result](results/A-028-public-shopify-hotspots.md), [frontend workstream](../workstreams/frontend-routes-and-components.md), [T-270](../tasks/T-270-fix-public-route-correctness-polish.md) | T-270 completed 2026-05-25 |
| F-134 | A-030 | High | Resolved | Admin artwork and collection deletion left stale reciprocal references. | A-030 found artwork deletion left deleted artwork IDs in user `favourites`/`watchlist` arrays, and collection deletion left deleted collection IDs in artwork `collections` arrays. T-264 updated previews and destructive routes so artwork deletes pull user saved-artwork references and collection deletes pull artwork collection references inside guarded/audited flows. | [A-030 result](results/A-030-admin-delete-integrity.md), [content/admin workstream](../workstreams/content-assets-and-admin-ops.md), [auth/admin workstream](../workstreams/auth-admin-and-permissions.md), [data/API workstream](../workstreams/data-models-and-api.md), [T-264](../tasks/T-264-clean-up-admin-reciprocal-delete-references.md) | T-264 completed 2026-05-25 |
| F-135 | A-031 | Medium | Resolved | Existing artwork image replacement was not available through the admin update workflow. | A-031 found the artwork update route could accept an optional replacement `image`, but `UpdateArtworkForm` did not expose replacement. T-271 added a replacement-image upload control using the existing signed Cloudinary upload path, visible upload-result errors, and focused form/route/upload-button coverage without deleting old assets. | [A-031 result](results/A-031-admin-content-controls.md), [content/admin workstream](../workstreams/content-assets-and-admin-ops.md), [testing workstream](../workstreams/testing-and-quality.md), [T-271](../tasks/T-271-add-admin-artwork-image-replacement.md) | T-271 completed 2026-05-25 |
| F-136 | A-031 | Medium | Resolved | Article and collection artwork relationship controls relied on manual ObjectIds with weak or silent feedback. | A-031 found article lookup failures and collection duplicate/not-found/ready states were not visible enough for operators. T-272 added visible empty, not-found, lookup-failure, duplicate, ready, unchanged, and no-op feedback while preserving manual ObjectId entry and submit contracts. | [A-031 result](results/A-031-admin-content-controls.md), [content/admin workstream](../workstreams/content-assets-and-admin-ops.md), [T-272](../tasks/T-272-improve-admin-artwork-relationship-feedback.md) | T-272 completed 2026-05-25 |
| F-137 | A-031 | Medium | Resolved | Blog and collection image controls still rely on raw URL paste with submit-time allowlist validation and no admin media picker. | A-031 found blog and collection image URL fields preview HTTP(S) strings before submit, but approved-host policy feedback happened only when create/update requests were submitted. T-303 scoped the next slice as pre-submit allowed-host feedback and preview gating, and T-306 implemented that focused operator-feedback fix for blog/collection create/update forms. Full media picker, generic Cloudinary upload reuse, Cloudinary lifecycle cleanup, route/schema changes, and content-image migration remain separate future work, not part of this resolved feedback scope. | [A-031 result](results/A-031-admin-content-controls.md), [T-303 result](results/T-303-admin-image-control-feedback.md), [T-306](../tasks/T-306-add-admin-content-image-url-feedback.md), [content/admin workstream](../workstreams/content-assets-and-admin-ops.md), [production risks](../risks/production-readiness.md) | T-306 completed 2026-05-26 |
| F-138 | A-031 | Low | Deferred | Comment and user read tabs lack search/filter controls for routine moderation or cleanup discovery. | A-031 found comment/user read tabs are pagination-only; this may cause manual ObjectId lookup friction, but it is lower priority after the completed media/relationship control slices and should be assigned only if operators report routine lookup friction. | [A-031 result](results/A-031-admin-content-controls.md), [content/admin workstream](../workstreams/content-assets-and-admin-ops.md) | Deferred by T-267 on 2026-05-25 |
| F-139 | A-032 | Medium | Resolved | Admin frontend pages relied on middleware JWT role only instead of persisted admin-role verification. | A-032 found admin APIs verified persisted MongoDB role through `requireApiAdmin()`, but admin dashboard rendering did not. T-273 added a server-only persisted-role frontend guard before the admin dashboard shell renders, redirecting stale admin JWTs after persisted demotion while preserving API guard behavior. | [A-032 result](results/A-032-auth-protected-boundaries.md), [auth/admin workstream](../workstreams/auth-admin-and-permissions.md), [architecture workstream](../workstreams/architecture-refactor-and-code-health.md), [T-273](../tasks/T-273-add-admin-frontend-persisted-role-guard.md) | T-273 completed 2026-05-25 |
| F-140 | A-032 | Medium | Resolved | `authOptionsImportBoundary.test.tsx` was stale after root layout no longer performed global DB/session work. | A-032 found the auth boundary suite failed because it expected root layout `dbConnect()` calls. T-263 updated the test to assert the current invariant: root layout renders without loading bcrypt and without requiring global DB/session modules or calls. | [A-032 result](results/A-032-auth-protected-boundaries.md), [architecture workstream](../workstreams/architecture-refactor-and-code-health.md), [testing workstream](../workstreams/testing-and-quality.md), [T-263](../tasks/T-263-repair-immediate-verification-drift.md) | T-263 completed 2026-05-25 |
| F-141 | A-032 | Low | Resolved | OAuth role/session behavior lacked a provider-specific propagation test. | Resolved by T-289: `credentialsRoleSession.test.ts` now covers a provider-shaped OAuth callback path with a Google OAuth account/profile fixture and adapter-defaulted `role: "user"`, proving the default user role reaches both JWT and session state without changing production auth behavior. | [A-032 result](results/A-032-auth-protected-boundaries.md), [auth/admin workstream](../workstreams/auth-admin-and-permissions.md), [testing workstream](../workstreams/testing-and-quality.md), [T-289](../tasks/T-289-add-oauth-role-session-propagation-test.md) | T-289 completed 2026-05-25 |
| F-142 | A-032 | Low | Resolved | Legacy session/admin helper files remained unused by active route boundaries and needed pruning. | T-290 classified all current `src/lib/session/*` helpers: `getUserIdFromSession.ts` and `requireAdminFrontendAccess.ts` are active. T-291 deleted the unused `createUserFromSession.ts`, `getUserFromSession.ts`, and `isAdmin.ts` helpers plus stale test-only references. The protected API guard inventory still bans direct helper use at protected API boundaries. | [A-032 result](results/A-032-auth-protected-boundaries.md), [architecture workstream](../workstreams/architecture-refactor-and-code-health.md), [auth/admin workstream](../workstreams/auth-admin-and-permissions.md), [T-290](../tasks/T-290-scope-legacy-session-helper-pruning.md), [T-291](../tasks/T-291-prune-unused-legacy-session-helpers.md) | T-291 completed 2026-05-26 |
| F-143 | A-033 | High | Partially mitigated | Monitoring provider/no-provider launch posture remains blocked, leaving no provider SDK, `instrumentation.ts`, source-map/release tracking, dashboard, uptime check, or alert automation. | A-033 found no monitoring provider dependency or app `instrumentation.ts`; ADR 0005 originally kept provider work blocked until owner/platform decision. On 2026-05-27 the owner approved Sentry and T-310 added the first Sentry SDK/instrumentation baseline. Source-map/release upload policy, alert routing, uptime checks, provider dashboard/operator ownership, credentialed smoke, provider-backed evidence, and broader tracing/replay/profiling decisions remain open. | [A-033 result](results/A-033-deployment-monitoring-smoke.md), [ADR 0005](../decisions/0005-monitoring-provider-decision.md), [deployment workstream](../workstreams/deployment-security-and-observability.md), [T-274](../tasks/T-274-prepare-production-ops-owner-decision-packet.md), [T-310](../tasks/T-310-implement-sentry-monitoring-baseline.md), [production risks](../risks/production-readiness.md) | T-310 completed 2026-05-27 |
| F-144 | A-033 | High | Partially mitigated | Incident authority, Vercel log/rollback ownership, credentialed smoke accounts, and public-smoke repository variables remain owner/platform blocked. | A-033 found the incident owner matrix and Vercel operator rows still blocked, complete smoke evidence cannot be produced without approved non-admin/admin smoke accounts and Vercel log access, and scheduled/detail public smoke needs non-secret owner-approved repo variables. On 2026-05-27 the owner created a local `VERCEL_TOKEN` for approved Vercel CLI/API deployment and targeted log inspection. On 2026-05-28 the owner accepted launch-safe production-ops defaults, configured `SMOKE_BASE_URL`, and approved optional public smoke values; T-325 verified unauthenticated public smoke with those values. Rollback, project settings, alias/domain changes, backup operator authority, credentialed smoke execution, and Vercel log evidence remain open. | [A-033 result](results/A-033-deployment-monitoring-smoke.md), [deployment workstream](../workstreams/deployment-security-and-observability.md), [testing workstream](../workstreams/testing-and-quality.md), [T-274](../tasks/T-274-prepare-production-ops-owner-decision-packet.md), [T-325](../tasks/T-325-align-public-smoke-product-not-found-check.md), [production risks](../risks/production-readiness.md) | T-325 completed 2026-05-28 |
| F-145 | A-033 | Medium | Resolved | Deployment runbook URL-variable requirements and one observability route test had stale expectations. | A-033 found `docs/runbooks/deployment.md` still listed retired same-app URL variables as minimum production values, and `apiRequestIdRoutes.test.ts` expected the old admin collection read error message. T-263 reconciled the deployment runbook with the environment runbook and updated the request-ID test expectation. | [A-033 result](results/A-033-deployment-monitoring-smoke.md), [deployment workstream](../workstreams/deployment-security-and-observability.md), [testing workstream](../workstreams/testing-and-quality.md), [T-263](../tasks/T-263-repair-immediate-verification-drift.md) | T-263 completed 2026-05-25 |
| F-146 | A-035 | Medium | Resolved | Root header navigation and `/prototype/home` added live MongoDB/Shopify reads to production build beyond the immediate account/project hard-failure task. | A-035 found the global header navigation performed MongoDB reads during prerendering and `/prototype/home` prerendered through live archive/shop data with degraded fallbacks. T-283 removed the root-header biography and collection navigation reads by making the global main nav use static route-root links. T-284 then made `/prototype/home` explicitly dynamic, keeping live prototype data at request time while avoiding prototype section reads during production static generation. | [A-035 result](results/A-035-build-external-dependency-map.md), [architecture workstream](../workstreams/architecture-refactor-and-code-health.md), [deployment workstream](../workstreams/deployment-security-and-observability.md), [frontend workstream](../workstreams/frontend-routes-and-components.md), [T-283](../tasks/T-283-make-root-header-nav-build-static-safe.md), [T-284](../tasks/T-284-set-prototype-home-production-build-policy.md) | T-284 completed 2026-05-25 |

## Reconciliation Notes

2026-05-26: T-291 completed the prepared F-142 source-pruning task by deleting
the unused legacy session/admin helpers and stale test-only references. F-142 is
now resolved.

2026-05-26: Orchestrator takeover review found the task/workstream trackers had
advanced past `docs/orchestration/state.md` and a few finding rows. Updated the
current orchestration handoff to mark T-289 and T-290 complete and route the next
agent-actionable work to T-291. Also credited T-284 in F-124/F-146 and T-288 in
F-126/R-034 so the remaining work is limited to intentional external build
evidence, owner/platform-blocked production operations, and the prepared T-291
source-pruning task.

2026-05-24: Reconciled A-024 into F-118 through F-122 and routed the loading
state UX implementation to the frontend and testing workstreams. The selected
implementation order is shared accessible loading primitives first, then
route-shaped shells for `/search`, `/artwork`, and `/shop/products`, followed
by programmatic navigation pending feedback, the admin dashboard `@main`
loading skeleton, and a targeted route-transition check. No runtime source was
changed by this reconciliation.

2026-05-25: T-267 reconciled A-027, A-028, A-030, A-031, A-032, A-033, A-034,
and A-035 plus completed T-263 through T-273 into F-123 through F-146. T-264,
T-265, T-266, T-269, T-270, T-271, T-272, and T-273 are recorded as resolved
implementation follow-ups. Remaining agent-actionable work is routed to T-274
through T-280 and workstream backlogs; owner/platform-blocked monitoring,
incident authority, Vercel operator, smoke-account, public-smoke variable, and
policy decisions remain separated from implementation work. No runtime source
...
2026-05-23: T-237 reconciled the post-A-022 Next.js efficiency options and
selected T-238 as the next F-111/R-012 implementation slice. T-238 should keep
its cache change limited to the `/collections` default redirect and route-local
collection navigation reads; provider/modal, sitemap, blog, static params, and
root header cache changes remain separate scopes.

2026-05-24: T-238 completed the collections default redirect cache proof. The
orchestrator verification reran focused collection/main-nav/cache-policy tests,
`npm run build`, prerender-manifest checks, and `git diff --check`. T-239 is
planned next to codify the sitemap's current one-hour ISR manifest behavior in
`src/app/sitemap.ts` rather than leaving it indirectly coupled to Shopify fetch
revalidation.

2026-05-24: T-239 completed explicit `/sitemap.xml` one-hour ISR ownership.
Focused sitemap/cache-policy tests, `npm run build`, prerender-manifest checks,
and `git diff --check` passed. T-240 was then planned as a docs-only blog
cache split scoping task because `/blog` and `/blog/[slug]` mix query-driven
list variants, primary post data, metadata/JSON-LD reads, and optional
comments; T-240 has since completed and selected T-241.

2026-05-24: T-240 completed the docs-only blog cache scoping pass and selected
T-241 as the next runtime slice. T-241 should stay limited to cached primary
blog detail reads for `/blog/[slug]` metadata, JSON-LD, and non-comments detail
rendering; optional comments, blog lists, route-level blog ISR, generated
params, cache tags, and provider work remain separate.

2026-05-24: T-241 completed the primary blog detail cache proof. Orchestrator
verification repeated the focused blog data, route-cache, loader, metadata,
breadcrumb, and artwork metadata Jest suites, `npm run build`, and
`git diff --check`; all passed, and build output kept `/blog` and
`/blog/[slug]` dynamic. It selected T-242 for only the default grouped `/blog`
list reads; sorted pages, public APIs, comments, route-level blog ISR, generated
params, cache tags, and mutation revalidation remained separate.

2026-05-24: T-242 completed the default grouped `/blog` list cache proof.
Orchestrator verification repeated the focused cached list wrapper,
`BlogListLoader`, and public route cache policy Jest suites, `npm run build`,
and `git diff --check`; all passed, and build output kept `/blog` dynamic.
It selected T-243 as a docs-only scoping task before any sorted blog list cache
or query parsing changes.

2026-05-24: T-243 completed the sorted blog list cache/query-hygiene scoping
pass and selected T-244 as the prerequisite implementation. Immediate sorted
list caching was rejected because `/blog` can pass `NaN` pages and the public
blog API accepts unbounded `page`/`limit` values. Orchestrator verification
repeated `git diff --check`; it passed.

2026-05-24: T-244 completed public blog list query normalization and bounds for
`/blog` and `GET /api/v2/public/blog`. Orchestrator verification repeated the
focused blog API, page, loader, continuous-loading, route-cache, and service
Jest suites, fixed a missing `BlogListSortBy` type re-export caught by
`npm run build`, then reran the focused tests, `npm run build`, and
`git diff --check`; all passed. It selected T-245 as the next sorted
first-page cache proof.

2026-05-24: T-245 completed the sorted `/blog` first-page cache proof.
Orchestrator verification repeated the focused cached list wrapper,
`BlogListLoader`, and public route cache policy Jest suites, `npm run build`,
and `git diff --check`; all passed, and build output kept `/blog` dynamic.
T-246 completed the bounded sorted page-2-plus scoping task, and T-247
completed the selected runtime proof for `latest`, `oldest`, and `featured`
pages 2-5 while keeping `popular` page 2-plus direct. T-248 completed the
next-cache-target scoping pass and selected T-249 as the next default artwork
browse runtime proof.
...
F-107. T-205 resolved F-108. T-206 resolved F-109. T-207 resolved F-110 by
documenting public route fallback patterns and replacing the `/project/aims`
generic inline loading fallback. T-208 completed the hosted Shopify
...
2026-05-23: A-022 was reconciled into F-111 through F-117, R-012/R-014/R-019/
R-025, the rendering architecture doc, the architecture/frontend/deployment
workstream backlogs, and planned task briefs T-230 through T-235. No runtime
implementation was performed by this reconciliation. The safe implementation
order is T-230 client import-boundary repair, T-231 middleware matcher
narrowing, T-232 public cache/freshness policy, T-233 first biography cache
proof route, T-234 client-provider island scoping, T-236 mobile drawer
lazy-loading, then lower-priority T-235 non-action `"use server"` cleanup.

2026-05-23: T-230 resolved F-112/R-025 by replacing `SignUpForm`'s broad
`@/lib/constants` runtime import with the narrow
`@/lib/constants/accountPrivacyAcknowledgement` import. The task handoff records
focused import-boundary/sign-up privacy tests and build passing. Continue the
A-022 implementation sequence with T-231 middleware matcher narrowing.

2026-05-23: T-231 resolved F-113 and further mitigated R-012 by narrowing
middleware matching to protected frontend/API prefixes only and tightening
protected route utility checks to exact-or-nested matching. The task handoff
records focused middleware, route utility, public-shell auth tests, and build
passing after one transient Next page-data retry. T-232 later completed the
public cache/freshness policy before any cache runtime change.

2026-05-23: T-232 resolved F-114 and partially mitigated F-111/R-012 by adding
the accepted public freshness matrix to the rendering architecture doc and
updating `publicRouteCachePolicy` coverage. Current runtime source remained
unchanged. T-233 is now scoped as the biography cache proof: cached
non-`fetch` service wrappers for biography article detail/navigation, a
10-minute stale window, no `generateStaticParams()`, and optional route-level
`revalidate` only for `/biography` default redirect if included and verified.

2026-05-23: T-233 partially mitigated F-111/R-012 by implementing the biography
cache proof. The route-local biography detail, metadata, structured data,
subnav, default redirect, and previous/next navigation paths now use cached
non-`fetch` wrappers with a 10-minute stale window. `/biography` is the only
route-level redirect ISR export added, `/biography/[slug]` remains dynamic, no
`generateStaticParams()` was added, and `MainNavLoader` stays on the direct
service to avoid propagating the proof cache window to unrelated static shells.
T-283 later replaced that direct root-header navigation read with static
route-root links.

2026-05-23: T-234 scoped F-115 without runtime changes. Build/source evidence
showed root layout client code still includes cross-cutting provider and mobile
drawer code, so the first safe runtime proof is T-236: lazy-load public mobile
search/navigation drawer implementations before attempting root provider moves.

2026-05-23: T-236 partially mitigated F-115 by lazy-loading the public mobile
search/navigation drawer bodies after first open intent. Root providers were
not moved, and the build showed the root layout client chunk shrink from the
T-234 `35,174` byte baseline to `27,772` bytes with separate lazy drawer
chunks. Future provider/modal moves remain separate work.

2026-05-23: T-235 resolved F-117 by removing non-action top-level
`"use server"` directives from ordinary `src/app`, `src/components`, and
utility modules. The remaining directives are limited to action/session helper
boundaries under `src/lib/actions` and `src/lib/session`, with source-hygiene
coverage in `clientServerImportBoundary.test.ts`.
