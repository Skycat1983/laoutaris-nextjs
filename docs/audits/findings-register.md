...
| F-115 | A-022 | Medium | Scoped | Global root client providers and large public client islands reduce how much public UI can stay purely server-rendered. | T-234 measured the current provider/client-island cost and found the root layout client chunk still carries cross-cutting session, modal, search drawer, and mobile navigation drawer code. It scoped T-236 as the first runtime proof: lazy-load the public mobile search/navigation drawer implementations while preserving root provider placement. | [A-022 result](results/A-022-nextjs-feature-utilization.md), [architecture workstream](../workstreams/architecture-refactor-and-code-health.md), [frontend workstream](../workstreams/frontend-routes-and-components.md), [T-234](../tasks/T-234-scope-public-client-provider-islands.md), [T-236](../tasks/T-236-lazy-load-public-mobile-navigation-drawers.md) | T-234 scoped 2026-05-23 |
...
| F-117 | A-022 | Low | Converted | Top-level `"use server"` appears in server component, loader, and layout files where `server-only` or ordinary Server Components would better express intent. | A-022 source search found top-level `"use server"` in files such as `src/components/views/Home.tsx`, `src/components/loaders/viewLoaders/BlogListLoader.tsx`, and `src/app/biography/layout.tsx`, even though the directive is primarily for server functions imported into clients. | [A-022 result](results/A-022-nextjs-feature-utilization.md), [architecture workstream](../workstreams/architecture-refactor-and-code-health.md), [frontend workstream](../workstreams/frontend-routes-and-components.md), [T-235](../tasks/T-235-remove-non-action-use-server-directives.md) | Reconciled 2026-05-23 |
