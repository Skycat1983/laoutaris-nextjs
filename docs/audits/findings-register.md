...
| F-052 | A-008 | Medium | Partially mitigated | Global API CORS and CSP policy are too broad for the production admin, auth, and commerce surface. | T-096 removed the global API CORS header rule from `next.config.mjs`, eliminating the wildcard-origin plus credential pairing and wildcard allowed request headers, and added baseline hardening headers plus low-risk CSP `object-src`, `base-uri`, `form-action`, and `frame-ancestors` directives. T-301 completed a source-backed CSP allowlist inventory and selected T-304 as a report-only tightening step before enforcement. Strict enforcement, dynamic route-level CORS, HSTS, CSP reporting, and monitoring follow-ups remain separate. | [A-008 result](results/A-008-security-headers-cors-logging.md), [T-096](../tasks/T-096-harden-baseline-security-headers-cors.md), [T-301 result](results/T-301-csp-allowlist-tightening-scope.md), [T-304](../tasks/T-304-add-report-only-csp-allowlist.md), [production risks](../risks/production-readiness.md), [deployment workstream](../workstreams/deployment-security-and-observability.md) | T-304 prepared 2026-05-26 |
...
| F-137 | A-031 | Medium | Converted | Blog and collection image controls still rely on raw URL paste with submit-time allowlist validation and no admin media picker. | A-031 found blog and collection image URL fields preview HTTP(S) strings before submit, but approved-host policy feedback happens only when create/update requests are submitted. T-303 scoped the next slice as pre-submit allowed-host feedback and preview gating for blog/collection image URL controls, not a full media picker or Cloudinary upload workflow. | [A-031 result](results/A-031-admin-content-controls.md), [T-303 result](results/T-303-admin-image-control-feedback.md), [T-306](../tasks/T-306-add-admin-content-image-url-feedback.md), [content/admin workstream](../workstreams/content-assets-and-admin-ops.md), [production risks](../risks/production-readiness.md) | T-306 prepared 2026-05-26 |
...
| F-143 | A-033 | High | Converted | Monitoring provider/no-provider launch posture remains blocked, leaving no provider SDK, `instrumentation.ts`, source-map/release tracking, dashboard, uptime check, or alert automation. | A-033 found no monitoring provider dependency or app `instrumentation.ts`; ADR 0005 keeps provider work blocked until owner/platform decision. T-274 is prepared as the owner decision packet, not implementation. | [A-033 result](results/A-033-deployment-monitoring-smoke.md), [ADR 0005](../decisions/0005-monitoring-provider-decision.md), [deployment workstream](../workstreams/deployment-security-and-observability.md), [T-274](../tasks/T-274-prepare-production-ops-owner-decision-packet.md), [production risks](../risks/production-readiness.md) | Reconciled by T-267 on 2026-05-25 |
