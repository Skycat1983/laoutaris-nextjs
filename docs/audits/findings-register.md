...
| F-114 | A-022 | High | Converted | `sitemap.ts`, `/biography`, and `/collections` are statically generated while reading mutable MongoDB/Shopify data, so their freshness is implicit and deploy-bound. | A-022 build output marks `/sitemap.xml`, `/biography`, and `/collections` static. `sitemap.ts` reads Article, Blog, Artwork, Collection, and Shopify product data; the default redirect pages query navigation services before redirecting. | [A-022 result](results/A-022-nextjs-feature-utilization.md), [production risks](../risks/production-readiness.md), [rendering architecture](../architecture/rendering-and-data-fetching.md), [deployment workstream](../workstreams/deployment-security-and-observability.md), [T-232](../tasks/T-232-define-public-cache-freshness-policy.md) | Reconciled 2026-05-23 |
| F-115 | A-022 | Medium | Converted | Global root client providers and large public client islands reduce how much public UI can stay purely server-rendered. | A-022 found `src/app/layout.tsx` wraps every route in `ClientContextBoundary`, which mounts `SessionProvider` and `GlobalFeaturesProvider`; build output still shows first-load JS around 177-183 kB on high-value public browse routes and 251 kB for the admin dashboard segment. | [A-022 result](results/A-022-nextjs-feature-utilization.md), [architecture workstream](../workstreams/architecture-refactor-and-code-health.md), [frontend workstream](../workstreams/frontend-routes-and-components.md), [T-234](../tasks/T-234-scope-public-client-provider-islands.md) | Reconciled 2026-05-23 |
...
narrowing, T-232 public cache/freshness policy, T-233 first ISR/cache proof
route, then lower-priority T-234 client-provider island scoping and T-235
non-action `"use server"` cleanup.
...
passing after one transient Next page-data retry. Continue with T-232 public
cache/freshness policy before any ISR/cache runtime change.
