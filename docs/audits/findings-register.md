...
| F-118 | A-024 | High | Converted | Dynamic public route transitions need segment-specific loading shells for artwork, search, and shop product browsing. | A-024 found only the root `src/app/loading.tsx` generic spinner, no route-local loading file or route-shaped Suspense fallback for `/artwork`, `/search`, or `/shop/products`, and better existing coverage on `/blog` through a section skeleton. | [A-024 result](results/A-024-loading-state-ux.md), [frontend workstream](../workstreams/frontend-routes-and-components.md), [architecture workstream](../workstreams/architecture-refactor-and-code-health.md), [testing workstream](../workstreams/testing-and-quality.md) | Reconciled 2026-05-24 |
| F-119 | A-024 | Medium | Converted | Programmatic search, artwork, and account navigations need pending feedback and should avoid full document navigation for internal routes. | A-024 found `Searchbar` and `SearchDrawerBody` call `router.push()` without pending feedback, the home artwork filter hero uses `window.location.href` for internal artwork search URLs, and account dropdown internal routes use raw anchors. | [A-024 result](results/A-024-loading-state-ux.md), [frontend workstream](../workstreams/frontend-routes-and-components.md) | Reconciled 2026-05-24 |
| F-120 | A-024 | Medium | Converted | Shared spinner and loading primitives need accessible status semantics and better public-route layout preservation. | A-024 found `PageLoading` renders full-viewport generic `Loading...` copy, `Spinner` lacks `role="status"`/`aria-live`/hidden status text, and shop/artwork localized spinners do not consistently mark busy regions. | [A-024 result](results/A-024-loading-state-ux.md), [frontend workstream](../workstreams/frontend-routes-and-components.md), [testing workstream](../workstreams/testing-and-quality.md) | Reconciled 2026-05-24 |
| F-121 | A-024 | Medium | Converted | Admin dashboard `@main` parallel route needs a matching loading skeleton. | A-024 found `src/app/admin/dashboard/@feed/loading.tsx` renders `FeedSkeleton`, while there is no `src/app/admin/dashboard/@main/loading.tsx`; the `@main` default file imports loading components but returns `null`. | [A-024 result](results/A-024-loading-state-ux.md), [frontend workstream](../workstreams/frontend-routes-and-components.md), [auth/admin workstream](../workstreams/auth-admin-and-permissions.md), [testing workstream](../workstreams/testing-and-quality.md) | Reconciled 2026-05-24 |
| F-122 | A-024 | Low | Converted | Route-transition loading behavior needs a targeted browser or component navigation check after loading shells exist. | A-024 found existing source/unit tests for local fallback patterns and client follow-up loading states, but no delayed route-transition check for `/artwork`, `/search`, or `/shop/products`. | [A-024 result](results/A-024-loading-state-ux.md), [testing workstream](../workstreams/testing-and-quality.md), [frontend workstream](../workstreams/frontend-routes-and-components.md) | Reconciled 2026-05-24 |
