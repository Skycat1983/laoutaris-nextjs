...
`BlogListLoader`, and public route cache policy Jest suites, `npm run build`,
and `git diff --check`; all passed, and build output kept `/blog` dynamic.
T-246 is planned as a docs-only scoping task before any sorted page-2-plus cache
expansion.
