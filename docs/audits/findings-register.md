...
2026-05-18: T-128 was prepared as the next F-081/R-019 implementation slice for
server actions and the development test-header session helper. It should
migrate scoped subscription, favourite, watchlist, and session-helper failure
logging to structured redacted server events without changing action return
contracts, saved-item mutation/revalidation behavior, or development-only test
header semantics.
