...
| R-002 | High | Auth/Admin | Admin role persistence, stable user identity/ownership, Cloudinary admin signing guard, stale `/protected`, legacy auth/session cleanup, and bootstrap/recovery remain open. | Open: A-004 complete; resolve F-042, F-043, F-044, F-045, and F-032 through the auth workstream. | Unassigned |
...
| R-008 | High | Assets/Security | Cloudinary upload signing lacks production-ready admin/API controls, environment documentation, and asset lifecycle policy. | Open: resolve F-044 and F-041 through auth, content/assets, deployment, and A-009. | Unassigned |
...
| R-027 | High | Deployment/Security | Server-only secrets can be exposed through Next config `env`; `MONGO_URI` is currently configured there. | Open: resolve F-046 by removing `MONGO_URI` from `next.config.mjs` and adding a guard against server-only env exposure. | Unassigned |
