...
| `VERCEL_URL` | Vercel deployment hostname used by preview server fetchers. | Platform-provided non-secret hostname. | Vercel preview and production while shared server API helpers remain in use. | Vercel platform owner. Vercel provides a hostname without protocol. T-086 removed the `/project` redirect dependency on this variable; shared server API helper URL policy remains separate work. |
