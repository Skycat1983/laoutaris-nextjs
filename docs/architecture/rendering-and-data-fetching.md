...
Noindex prototype routes that read live archive or commerce data should not add
hidden production-build data requirements. `/prototype/home` is explicitly
dynamic: it keeps the owner-review experience on live MongoDB/Shopify-backed
prototype data at request time, but it is not statically generated during
`npm run build`.
...
10, `filterMode: "ALL"`, no taxonomy filters, and no `sortColor`; T-251
expands that same fixed unfiltered `mostRecent` cache boundary to pages 2-5
with limit 10 while keeping page 6-plus and broader browse variants direct.
`/artwork` remains route-dynamic. Other public route families remain free of route-level
...
| --- | --- | --- | --- |
| Stable static shell | `/biography`, `/collections`, `/project`, `/project/aims`, `/project/film`, `/shop` | Leave without forced dynamic config or generated params. `/biography` and `/collections` are the only current shell routes with route-level ISR, using accepted 10-minute cache windows for their default redirects. The other shell routes remain without ISR config. | Keep root layout and middleware free of global DB/session work. Revisit if route-local loaders are added. |
| Query-driven public browse/search | `/artwork`, `/blog`, `/search`, `/shop/products`, `/project/contact` | `dynamic = "force-dynamic"` is explicit. Query params select filters, sort, pagination, search terms, or product enquiry context and should not be silently treated as static. The only current artwork browse-list service cache exception is the T-249/T-251 fixed unfiltered `mostRecent` `/artwork` server-rendered list for pages 1-5 limit 10, which does not add route ISR. | A future ISR/static task must first define canonical query variants or split static shells from query results. |
| DB-backed public detail/redirect | `/artwork/[artworkId]`, `/biography/[slug]`, `/blog/[slug]`, `/collections/[slug]`, `/collections/[slug]/[artworkId]`, `/project/about` | `dynamic = "force-dynamic"` is explicit. These routes read owner-controlled MongoDB content at request time for detail content, redirect targets, metadata, JSON-LD, or saved-item state. `/project/about` is included because it renders project article content through `ArticleLoader`. | Static params and ISR are deferred until content freshness, not-found behavior, and build-time DB access are approved. |
...
| Noindex prototype routes with live data | `/prototype/home` | `dynamic = "force-dynamic"` is explicit. The prototype keeps live owner-review data at request time while avoiding static-generation MongoDB/Shopify reads during production build. | Move to static fixtures or remove from production only through a separate prototype route policy task. |
...
| Biography article list/detail | Route-local biography navigation, `/biography`, `/biography/[slug]`, article metadata, JSON-LD, and previous/next navigation now use cached biography service wrappers with a 10-minute stale window. `/biography/[slug]` remains explicitly dynamic, and no biography route uses `generateStaticParams()`. The global header links to `/biography` without resolving a first article. | A short stale window is acceptable for biography article text, metadata, JSON-LD, and previous/next navigation. Missing primary articles still map to the existing not-found/error contract, and service failures must continue to log and fail through the App Router error boundary. | T-233 first proof route: biography. Cached non-`fetch` service wrappers are implemented for biography article detail and route-local biography navigation with a 10-minute TTL. Do not add `generateStaticParams()` in the first proof. Keep route-level `revalidate` limited to `/biography` default redirect. `MainNavLoader` must stay static-safe: first-item biography resolution belongs to the `/biography` route, not to the root header. |
| Collections navigation/detail | Route-local collections subnav and `/collections` default redirect now use a cached collection navigation wrapper with a 10-minute stale window. `/collections/[slug]` and `/collections/[slug]/[artworkId]` remain explicitly dynamic, and no collections route uses `generateStaticParams()`. The global header links to `/collections` without resolving a first collection or first artwork. | A short stale window is acceptable for route-local collection navigation and the default redirect target. Collection detail rendering, collection artwork membership, pagination, Shopify product links, and saved-item state remain request-time behavior. | T-238 second proof route: collections. Cached non-`fetch` service wrappers are implemented only for collection navigation reads that derive redirect/subnav targets. Keep route-level `revalidate` limited to `/collections` default redirect. `MainNavLoader` must stay static-safe: first-item collection resolution belongs to the `/collections` route, not to the root header. |
| Artwork browse/detail | `/artwork`, `/artwork/[artworkId]`, and collection-scoped artwork detail are explicitly dynamic. Browse is filter/sort/page driven, but T-249/T-251 cache only the server-rendered unfiltered default `/artwork` list shape: `mostRecent` pages 1-5 limit 10, `filterMode: "ALL"`, no taxonomy filters, and no `sortColor`. Filtered browse variants, page 6-plus, non-default limits, `mostPopular`, `mostFeatured`, `colorProximity`, public artwork APIs/fetchers, browser follow-up fetches, detail reads, metadata/JSON-LD detail reads, saved-item state, and Shopify product-link reads stay direct. | Keep routes dynamic. A short stale window is accepted only for the fixed unfiltered default browse list pages because it is owner-controlled MongoDB archive content with no selected-branch session or Shopify coupling. | T-249 artwork proof: cached non-`fetch` service wrapper for the default page 1 browse list only. T-251 artwork proof: fixed cached wrappers for unfiltered `mostRecent` pages 2-5 only. Do not add route-level ISR, `generateStaticParams()`, cache tags, mutation revalidation, filtered cache variants, page 6-plus caching, API caching, detail caching, saved-item caching, or Shopify product-link caching. |
...
T-233, T-238, T-241, T-242, T-244, T-245, T-247, T-249, and T-251 verification prove the current
...
4. Pilot route-family caching/ISR in narrow slices after freshness is accepted.
   ...
   cache propagation. T-249 proved the default `/artwork` browse list page 1 cache
   ...
   T-251 expanded that artwork browse cache only to fixed unfiltered
   `mostRecent` pages 2-5 with limit 10 while leaving page 6-plus direct.
...
- `src/lib/data/services/getCachedArtworkListData.ts` wraps only the fixed
  unfiltered `mostRecent` `/artwork` server-rendered browse list reads for
  pages 1-5, limit 10, `filterMode: "ALL"`, no taxonomy filters, and no
  `sortColor`, with a 10-minute non-`fetch` cache for the T-249/T-251 proofs.
  Filtered variants, page 6-plus, non-default limits, `mostPopular`,
  `mostFeatured`, `colorProximity`, public artwork API routes, browser
  follow-up fetches, artwork detail routes, metadata/JSON-LD detail reads,
  saved-item/session-aware reads, and Shopify product-link reads remain on
  direct services.
...
- `src/lib/data/services/getArticleNavigationList.ts` is shared by
  `BiographySubnavLoader` and
  `GET /api/v2/public/navigation/articles/[section]`, so route-local biography
  navigation no longer self-fetches the same app. The biography default
  redirect page and `ArticleLoader` previous/next navigation path also use this
  service directly. `MainNavLoader` intentionally does not use this service:
  the root header renders static `/biography` and `/collections` route-root
  links so otherwise static shells do not perform live navigation reads during
  build.
...
- `src/lib/data/services/getCachedBiographyArticleData.ts` wraps the biography
  ...
  non-`fetch` cache for the T-233 proof. API routes continue to use the direct
  services, and global `MainNavLoader` uses static route-root links so the proof
  does not broaden into unrelated route output.
- `src/lib/data/services/getCachedCollectionNavigationData.ts` wraps
  ...
  the T-238 proof. API routes and collection detail routes continue to use the
  direct services, and global `MainNavLoader` uses static route-root links so
  the proof does not broaden into unrelated route output.
