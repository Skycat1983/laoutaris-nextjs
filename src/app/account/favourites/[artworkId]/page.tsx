...
/**
 * @fileoverview
 * This Next.js server-side component handles the `/account/favourites/[artworkId]` route.
 *
 * - **Purpose:**
 *   The `FavouritedArtwork` component is responsible for fetching and rendering a specific artwork
 *   from a user's favourites list. It ensures that only authenticated users can access their
 *   favourited artworks and provides a seamless experience by displaying the artwork using the
 ...
 *     4. **Artwork Fetching:**
 *        Calls the `fetchUserFavourite` function to retrieve the specific artwork (`artworkId`)
 *        from the authenticated user's favourites.
 *     5. **Error Handling:**
 ...
 *     - `fetchUserFavourite`: Fetches a specific artwork from the user's favourites.
 ...
 * - **Notes:**
 *   - **Security:**
 *     Ensures that only authenticated users can access their favourites, preventing unauthorized access to artwork details.
 *   - **Performance:**
 ...
 *   - **Data Integrity:**
 *     Assumes that each user has a unique and correctly formatted favourites list associated with their email address.
 *   - **Developer Notes:**
 ...
 *
 * - **Usage Example:**
 *   When a user navigates to `/account/favourites/60d5ec49f8d2e4567890abcd`, the `FavouritedArtwork` component:
 *   1. Connects to the database.
 *   2. Retrieves and validates the user's session.
 *   3. Fetches the artwork with ID `60d5ec49f8d2e4567890abcd` from the user's favourites.
